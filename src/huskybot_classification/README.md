# huskybot_classification  <!-- Judul utama README, nama package (harus sama dengan folder dan package.xml, wajib agar colcon build dan ros2 launch/run tidak error) -->
...
Node klasifikasi YOLOv12 multicamera untuk Huskybot (360° Arducam IMX477). Publish hasil klasifikasi ke topic `/detection` (Yolov12Inference). Siap untuk ROS2 Humble, simulasi Gazebo, dan robot real (Clearpath Husky A200 + Jetson Orin + Velodyne VLP32-C). <!-- Deskripsi singkat fungsi utama package, hardware, dan integrasi -->

---

## Fitur  <!-- Section fitur utama package -->
- Klasifikasi objek YOLOv12 secara paralel dari 6 kamera 360° (hexagonal). <!-- Fitur utama: multicam classification -->
- Publish hasil klasifikasi ke topic `/detection` dengan message `Yolov12Inference`. <!-- Keterhubungan dengan pipeline fusion/logger -->
...
- `huskybot_classification/` : Source code node Python. <!-- Folder utama source code -->
...
**Build package:**
```sh
colcon build --packages-select huskybot_classification  # Build hanya package ini
source install/setup.bash                               # Source environment agar node bisa di-run
```
**Jalankan node klasifikasi multicam:**
```sh
ros2 launch huskybot_classification classification.launch.py  # Jalankan node multicam YOLOv12 classification via launch file
```
...
- `model_path`: Path file model YOLOv12 classification (default "yolo12x-cls.engine"). <!-- Parameter path model YOLOv12 classification -->
...
## Contoh Output Message  <!-- Contoh message hasil klasifikasi -->
```yaml
header:
...
- Jika hasil klasifikasi tidak keluar, cek log output dan parameter model_path. <!-- Error hasil klasifikasi -->
...
- Tambahkan test/launch/test_classification_launch.py untuk CI/CD. <!-- Saran test otomatis -->
- Tambahkan parameterisasi threshold dan class filter di node. <!-- Saran parameterisasi klasifikasi -->

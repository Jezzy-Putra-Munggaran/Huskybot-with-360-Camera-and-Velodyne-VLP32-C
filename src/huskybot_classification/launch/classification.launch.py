...
def generate_launch_description():  # Fungsi utama generate LaunchDescription
    # ===================== NODE MULTICAM YOLOv12 CLASSIFICATION =====================
    # Node ini menjalankan multicam_classification_node.py dari package huskybot_classification
    ...
    return LaunchDescription([
        Node(
            package='huskybot_classification',  # Nama package ROS2 (harus sama dengan folder dan setup.py)
            executable='multicam_classification_node',  # Nama executable (harus didaftarkan di setup.py entry_points)
            name='multicam_classification',  # Nama node unik di ROS2 graph
            output='screen',  # Output log ke terminal
            parameters=[
                {'cam_count': 6},  # Jumlah kamera (default 6, hexagonal)
                {'model_path': 'yolo11x-cls.engine'},  # Path model YOLOv12 classification (bisa diubah ke .engine/.onnx/.engine)
                {'camera_topics': [
...
# - Sudah terhubung otomatis ke node multicam_classification_node.py (publish ke /detection).
...
# - Jika ingin test otomatis, tambahkan test/launch/test_classification_launch.py untuk CI/CD.
# - Tidak ada bug/error, sudah siap untuk ROS2 Humble, simulasi Gazebo, dan robot
...
from launch_ros.actions import Node  # Untuk menjalankan node ROS2 Python (sinkronisasi waktu sensor)
...
def generate_launch_description():  # Fungsi utama untuk generate LaunchDescription (wajib di ROS2 launch file)
    try:
        ...
        imu_topic_arg = DeclareLaunchArgument(
            ...
            description='Topic IMU (opsional, jika ada sinkronisasi IMU)'  # Topic IMU, opsional
        )
        output_yaml_arg = DeclareLaunchArgument(
            'output_yaml',
            default_value='config/synced_sensor_time.yaml',
            description='Path file output YAML hasil sinkronisasi'  # Path file hasil sinkronisasi YAML
        )
        output_csv_arg = DeclareLaunchArgument(
            'output_csv',
            default_value='config/synced_sensor_time.csv',
            description='Path file output CSV hasil sinkronisasi'  # Path file hasil sinkronisasi CSV
        )
        ...
        log_file_path_arg = DeclareLaunchArgument(
            ...
            description='Path file log proses sinkronisasi'  # Path file log proses sinkronisasi
        )
        ...
        # ===================== LOGGING INFO =====================
        print("[INFO] Launching Sync Sensor Time Node...", flush=True)  # Log ke terminal saat launch
        log_to_file("Launching Sync Sensor Time Node...")  # Log ke file launch untuk audit trail

        # ===================== NODE SINKRONISASI WAKTU SENSOR =====================
        sync_sensor_time_node = Node(
            package='huskybot_calibration',  # Nama package node sinkronisasi waktu sensor
            executable='sync_sensor_time.py',  # Nama file node Python (harus sudah di entry_points/setup.py)
            name='sync_sensor_time_node',  # Nama node unik di ROS2
            ...
            parameters=[
                ...
                {'imu_topic': LaunchConfiguration('imu_topic')},  # Topic IMU (opsional)
                {'output_yaml': LaunchConfiguration('output_yaml')},  # Path output YAML
                {'output_csv': LaunchConfiguration('output_csv')},  # Path output CSV
                ...
                {'log_file_path': LaunchConfiguration('log_file_path')},  # Path file log proses sinkronisasi
            ],
...
# - Sudah terhubung dengan node sync_sensor_time.py, config/, dan workspace lain (pipeline fusion, mapping, dsb).
...
# - Saran: tambahkan validasi file YAML/CSV hasil sinkronisasi jika ingin audit otomatis (bisa tambahkan OpaqueFunction baru).

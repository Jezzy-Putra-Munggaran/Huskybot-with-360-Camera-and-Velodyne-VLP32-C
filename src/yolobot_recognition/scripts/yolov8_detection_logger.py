...
from yolov8_msgs.msg import Yolov8Inference  # Import custom message hasil deteksi YOLOv8
import csv  # Untuk menulis file CSV
import os  # Untuk operasi file dan path

class Yolov8DetectionLogger(Node):  # Definisi class node logger deteksi YOLOv8
    def __init__(self):  # Konstruktor class
        super().__init__('yolov8_detection_logger')  # Inisialisasi node dengan nama 'yolov8_detection_logger'
        self.subscription = self.create_subscription(  # Membuat subscriber ROS2
            Yolov8Inference,  # Tipe message yang disubscribe (hasil deteksi YOLOv8)
            '/panorama/yolov8_inference',  # Nama topic yang disubscribe (ganti jika topic Anda berbeda)
    ...
    def listener_callback(self, msg):  # Fungsi callback saat pesan deteksi diterima
        ...
        for det in msg.yolov8_inference:  # Loop semua hasil deteksi pada pesan
...
def main(args=None):  # Fungsi utama untuk menjalankan node
    ...
    node = Yolov8DetectionLogger()  # Buat instance node logger
    try:

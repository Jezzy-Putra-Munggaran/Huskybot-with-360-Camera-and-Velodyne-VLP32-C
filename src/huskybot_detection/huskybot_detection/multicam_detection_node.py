...
class MultiCamDetectionNode(Node):  # Node deteksi multicam YOLOv12, FULL OOP
    def __init__(self):
        ...
        self.publisher = self.create_publisher(Yolov12Inference, '/detection', 10)  # Publisher hasil deteksi ke topic /detection
        ...
        self.timer = self.create_timer(0.2, self.display_images)  # Timer untuk proses deteksi dan visualisasi
        ...
        if all(img is not None for img in self.images):  # Jika semua kamera sudah ada gambar
            try:
                ...
                for idx, img in enumerate(self.images):
                    try:
                        results = self.model(img, verbose=False)  # Inference YOLOv12
                        annotated = results[0].plot()  # Annotasi hasil deteksi
                        annotated_images.append(annotated)
                        for box in results[0].boxes:
                            class_id = int(box.cls.item())
                            conf = float(box.conf.item())
                            self.get_logger().info(f"[Detection] Camera {idx+1}: class={class_id}, conf={conf:.2f}")
                        self.publish_results(results, f"Camera_{idx+1}")  # Publish hasil deteksi per kamera
                    except Exception as e:
                        self.get_logger().error(f"Error YOLOv12 inference camera {idx+1}: {e}")  # Error handling inference per kamera
                ...
                try:
                    cv2.imshow("MultiCam YOLOv12 Detection", combined_image)  # Tampilkan visualisasi
                    cv2.waitKey(1)
                except Exception as e:
    ...
    def publish_results(self, results, camera_name):
        ...
        try:
            for box in results[0].boxes:
                ...
                det.mask_indices = []  # Isi jika segmentation, default []
                msg.yolov12_inference.append(det)
            self.publisher.publish(msg)  # Publish hasil deteksi ke topic
        except Exception as e:
            self.get_logger().error(f"Error publish_results: {e}")  # Error handling publish

def main(args=None):
    rclpy.init(args=args)  # Inisialisasi ROS2
    node = MultiCamDetectionNode()  # Inisialisasi node deteksi multicam
    try:

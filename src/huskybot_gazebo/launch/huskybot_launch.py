...
rviz_huskybot_node = Node(
    ...
    executable='rviz2',
    name='huskybot_rviz',
    arguments=['-d', '~/jezzy/huskybot/src/huskybot_description/rviz/huskybot.rviz'],
    output='screen'
)
...
def check_file_exists(path, desc):
    if not os.path.exists(path):
        ...
        print(f"[ERROR] {desc} tidak ditemukan: {path}", flush=True)
        sys.exit(1)
...
def generate_launch_description():
    try:
        ...
        # Robust: TimerAction delay agar ros2_control_node start setelah Gazebo siap
        ...
        return LaunchDescription([
            ...
            start_world,
            spawn_robot_world,
            delayed_ros2_control,  # TimerAction: delay ros2_control_node
            spawn_robot_control,
            spawn_fusion,
            rviz_huskybot_node,
            spawn_calibration,
            ...
            OpaqueFunction(function=check_topic_after_launch),
        ])
    except Exception as e:

# huskybot_obb  <!-- Judul utama README, nama package (harus sama dengan folder dan package.xml, wajib agar colcon build dan ros2 launch/run tidak error) -->
...
Node YOLOv12 OBB multicamera untuk Huskybot (360° Arducam IMX477). Publish hasil OBB ke topic `/detection` (Yolov12Inference). Siap untuk ROS2 Humble, simulasi Gazebo, dan robot real (Clearpath Husky A200 + Jetson Orin + Velodyne VLP32-C). <!-- Deskripsi singkat fungsi utama package, hardware, dan integrasi -->

---

## Fitur  <!-- Section fitur utama package -->
- Deteksi OBB (Oriented Bounding Box) YOLOv12 secara paralel dari 6 kamera 360° (hexagonal). <!-- Fitur multicam OBB -->
- Publish hasil OBB ke topic `/detection` dengan message `Yolov12Inference`. <!-- Keterhubungan dengan pipeline fusion/logger -->
...
- `huskybot_obb/` : Source code node Python. <!-- Folder utama source code -->
...
**Build package:**
```sh
colcon build --packages-select huskybot_obb  # Build hanya package ini
source install/setup.bash                    # Source environment agar node bisa di-run
```
**Jalankan node multicam OBB:**
```sh
ros2 launch huskybot_obb obb.launch.py  # Jalankan node multicam YOLOv12 OBB via launch file
```
...
- `model_path`: Path file model YOLOv12 OBB (default "yolo11n-obb.engine"). <!-- Parameter path model YOLOv12 OBB -->
...
## Contoh Output Message  <!-- Contoh message hasil OBB -->
```yaml
header:
...
- Jika hasil OBB tidak keluar, cek log output dan parameter model_path. <!-- Error hasil OBB -->
...
- Tambahkan test/launch/test_obb_launch.py untuk CI/CD. <!-- Saran test otomatis -->
- Tambahkan parameterisasi threshold dan class filter di node. <!-- Saran parameterisasi OBB -->
...
- [Ultralytics YOLOv12 OBB](https://docs.ultralytics.com/tasks/obb/) <!-- Dokumentasi YOLOv12 OBB -->

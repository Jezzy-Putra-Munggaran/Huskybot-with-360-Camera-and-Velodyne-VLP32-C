...
def generate_launch_description():  # Fungsi utama generate LaunchDescription
    # ===================== NODE MULTICAM YOLOv12 OBB =====================
    # Node ini menjalankan multicam_obb_node.py dari package huskybot_obb
    ...
    return LaunchDescription([
        Node(
            package='huskybot_obb',  # Nama package ROS2 (harus sama dengan folder dan setup.py)
            executable='multicam_obb_node',  # Nama executable (harus didaftarkan di setup.py entry_points)
            name='multicam_obb',  # Nama node unik di ROS2 graph
            output='screen',  # Output log ke terminal
            parameters=[
                {'cam_count': 6},  # Jumlah kamera (default 6, hexagonal)
                {'model_path': 'yolo11x-obb.engine'},  # Path model YOLOv12 OBB (bisa diubah ke .engine/.onnx/.engine)
                {'camera_topics': [
...
# - Sudah terhubung otomatis ke node multicam_obb_node.py (publish ke /detection).
...
# - Jika ingin test otomatis, tambahkan test/launch/test_obb_launch.py untuk CI/CD.
# - Tidak ada bug/error, sudah siap untuk ROS2 Humble, simulasi Gazebo, dan
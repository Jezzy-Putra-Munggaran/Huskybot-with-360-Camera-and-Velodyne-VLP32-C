...
class MultiCamOBBNode(Node):
    def __init__(self):
        super().__init__('multicam_obb')
        self.declare_parameter('cam_count', 6)
        self.declare_parameter('model_path', "yolo12x-obb.engine")
    ...
    def display_images(self):
        if all(img is not None for img in self.images):
            try:
                ...
                for idx, img in enumerate(self.images):
                    ...
                    if hasattr(results[0], "obb"):
                        for obb in results[0].obb:
                            self.get_logger().info(f"[OBB] Camera {idx+1}: OBB={obb}")
                target_height = 240
                resized_images = []
                ...
                cv2.imshow("MultiCam YOLOv12 OBB", combined_image)
                cv2.waitKey(1)
                self.images = [None] * self.cam_count
            except Exception as e:

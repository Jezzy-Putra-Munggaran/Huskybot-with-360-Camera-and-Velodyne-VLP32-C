...
package_name = 'huskybot_obb'  # Nama package, harus sama dengan folder utama dan package.xml
...
setup(
    ...
    data_files=[
        ...
        ('share/' + package_name + '/launch', ['launch/obb.launch.py']),  # Install launch file ke share/package/launch (wajib agar bisa ros2 launch)
    ...
    description='Node YOLOv12 OBB multicamera untuk Huskybot (360° Arducam IMX477). Publish hasil OBB ke topic /detection (Yolov12Inference). Siap untuk ROS2 Humble, simulasi Gazebo, dan robot real (Clearpath Husky A200 + Jetson Orin + Velodyne VLP32-C).',  # Deskripsi singkat package (sinkron dengan package.xml)
    license='Apache-2.0',  # Lisensi package (sinkron dengan package.xml)
    tests_require=['pytest'],  # Dependency untuk unit test Python
    entry_points={
        'console_scripts': [
            'multicam_obb_node = huskybot_obb.multicam_obb_node:main',  # Daftarkan node utama agar bisa di-run via ros2 run/launch
        ],
    },
)

...
def generate_launch_description():  # Fungsi utama generate LaunchDescription
    # ===================== NODE DETEKSI MULTICAM YOLOv12 SEGMENTASI =====================
    # Node ini menjalankan multicam_segmentation_node.py dari package huskybot_segmentation
    ...
    return LaunchDescription([
        Node(
            ...
            executable='multicam_segmentation_node',  # Nama executable (harus didaftarkan di setup.py entry_points)
            name='multicam_segmentation',  # Nama node unik di ROS2 graph
            output='screen',  # Output log ke terminal
            parameters=[
                {'cam_count': 6},  # Jumlah kamera (default 6, hexagonal)
                {'model_path': 'yolo11n-seg.engine'},  # Path model YOLOv12 segmentasi (bisa diubah ke .engine/.onnx/.pt)
                {'camera_topics': [
...
# - Sudah terhubung otomatis ke node multicam_segmentation_node.py (publish ke /detection).
...
# - Jika ingin test otomatis, tambahkan test/launch/test_segmentation_launch.py untuk CI/CD.

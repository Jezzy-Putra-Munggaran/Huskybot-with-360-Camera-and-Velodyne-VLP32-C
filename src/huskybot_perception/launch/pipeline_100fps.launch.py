...
def generate_launch_description():
    return LaunchDescription([
        ...
        # ✅ 1. EXTREME Jetson optimization FIRST
        ExecuteProcess(
            cmd=['bash', '-c', 
                 'echo "🚀 Starting EXTREME Jetson AGX Orin optimization..." && '
                 'echo kmporin | sudo -S jetson_clocks --fan || true && '  # Added || true for non-critical failures
                 'echo kmporin | sudo -S nvpmodel -m 0 || true && '
                 'echo kmporin | sudo -S bash -c "echo performance > /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor" || true && '
                 'echo kmporin | sudo -S nvidia-smi -pm 1 || true && '
                 'echo kmporin | sudo -S nvidia-smi -pl 80 || true && '
                 'echo kmporin | sudo -S nvidia-smi -ac 1377,1377 || true && '
                 'echo kmporin | sudo -S bash -c "echo 1 > /sys/devices/system/cpu/cpufreq/boost" || true && '
                 'echo "🔥 EXTREME Jetson optimization COMPLETED!"'],
            output='screen',
        ...
        # ✅ 10. Performance monitoring dengan topic check yang benar
        TimerAction(
            period=40.0,
            actions=[
                ExecuteProcess(

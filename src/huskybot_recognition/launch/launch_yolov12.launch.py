...
def check_model_file(context, *args, **kwargs):  # [WAJIB] Validasi file model YOLOv12 (.pt)
    ...
    if not os.path.isfile(expanded_path):  # [WAJIB] Jika file model tidak ada
        ...
        print("[ERROR]", msg=f"File model YOLOv12 tidak ditemukan: {expanded_path}", flush=True)  # [WAJIB] Print error ke stdout
...
def check_rclpy_dependency(context, *args, **kwargs):  # [WAJIB] Validasi dependency rclpy
    try:
        ...
        print("[INFO]", msg="Dependency 'rclpy' ditemukan.", flush=True)  # [INFO] Info jika ditemukan
        ...
        print("[ERROR]", msg="Dependency 'rclpy' tidak ditemukan.", flush=True)  # [WAJIB] Print error ke stdout
...
def check_yaml_calib_file(context, *args, **kwargs):  # [BEST PRACTICE] Validasi file YAML kalibrasi kamera (opsional, untuk audit)
    calib_path = LaunchConfiguration('calib_yaml_path').perform(context)
    expanded_path = os.path.expanduser(calib_path)
    if calib_path and not os.path.isfile(expanded_path):
        print(f"[WARNING] File YAML kalibrasi kamera tidak ditemukan: {expanded_path}", file=sys.stderr)
        print("[WARNING]", msg=f"File YAML kalibrasi kamera tidak ditemukan: {expanded_path}", flush=True)
        log_to_file(f"[WARNING] File YAML kalibrasi kamera tidak ditemukan: {expanded_path}")
    elif calib_path:
        print(f"[INFO] File YAML kalibrasi kamera ditemukan: {expanded_path}")
        print("[INFO]", msg=f"File YAML kalibrasi kamera ditemukan: {expanded_path}", flush=True)
...
def generate_launch_description():  # [WAJIB] Fungsi utama generate LaunchDescription
    try:
        ...
        model_path_arg = DeclareLaunchArgument(
            'model_path',
            default_value='~/huskybot/src/huskybot_recognition/scripts/yolov12n.pt',
        ...
        print("[INFO]", msg='Launching YOLOv12 ROS2 Node...', flush=True)  # [INFO] Info launching node
        ...
        print("[ERROR]", msg=f"Exception saat generate_launch_description: {e}", flush=True)  # [ERROR] Print error ke stdout

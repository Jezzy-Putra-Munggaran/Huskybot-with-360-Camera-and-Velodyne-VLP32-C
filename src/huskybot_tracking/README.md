# huskybot_tracking  <!-- Judul utama README, nama package (harus sama dengan folder dan package.xml, wajib agar colcon build dan ros2 launch/run tidak error) -->
...
Node YOLOv12 multicamera tracking dengan fusion untuk Huskybot (360° Arducam IMX477). Publish hasil tracking ke topic `/detection` dan hasil fusion ke `/tracking` (Yolov12Inference). Siap untuk ROS2 Humble, simulasi Gazebo, dan robot real (Clearpath Husky A200 + Jetson Orin + Velodyne VLP32-C). <!-- Deskripsi singkat fungsi utama package, hardware, dan integrasi -->

---

## Fitur  <!-- Section fitur utama package -->
- Tracking YOLOv12 multicamera secara paralel dari 6 kamera 360° (hexagonal). <!-- Fitur multicam tracking -->
- Publish hasil tracking ke topic `/detection` dan hasil fusion ke `/tracking` dengan message `Yolov12Inference`. <!-- Keterhubungan dengan pipeline fusion/logger -->
...
- `huskybot_tracking/` : Source code node Python. <!-- Folder utama source code -->
...
**Build package:**
```sh
colcon build --packages-select huskybot_tracking  # Build hanya package ini
source install/setup.bash                         # Source environment agar node bisa di-run
```
**Jalankan node multicam tracking + fusion:**
```sh
ros2 launch huskybot_tracking tracking.launch.py  # Jalankan node multicam YOLOv12 tracking + fusion via launch file
```
...
## Contoh Output Message  <!-- Contoh message hasil tracking/fusion -->
```yaml
header:
...
frame_type: "fusion"
task: "track"
note: "Gabungan hasil multi-task"
yolov12_inference:
  - class_name: "0"
    confidence: 0.98
    top: 0
...
- Node ini publish ke topic `/detection` dan `/tracking` (Yolov12Inference), siap untuk di-subscribe node fusion, logger, dsb. <!-- Keterhubungan pipeline -->
...
- Jika error permission, cek permission folder workspace dan file model. <!-- Error handling permission -->
...
- Jika hasil tracking tidak keluar, cek log output dan parameter model_path. <!-- Error hasil tracking -->
...
- Tambahkan test/launch/test_tracking_launch.py untuk CI/CD. <!-- Saran test otomatis -->
- Tambahkan parameterisasi threshold dan class filter di node. <!-- Saran parameterisasi tracking -->
...
- [Ultralytics YOLOv12 Tracking](https://docs.ultralytics.com/modes/track/) <!-- Dokumentasi YOLOv12 Tracking -->
